...
(defun alist->vcf3.0-format (alist)
  (format t "Processing: ~A~%" (get-vcf-field-value 'full-name alist))
  
  (with-output-to-string (out-stream)
    (format out-stream "BEGIN:VCARD~%VERSION:3.0")
   ...
   (let ((url (get-vcf-field-value 'url alist)))
     (if (> (length url) 0)
         (format out-stream "~%URL:~A" url)))
...
;; home-address
;; note
(defun csv->vcf (fields csv-filepath &key note org section-mark use-quoted-printable (gen-qr t))
      ...
      (let ((records (restart-case
        ...
        (loop for alist in records
           do (handler-bind ((no-pinyin-for-hanzi-error
                              #'use-manual-value))
                (let ((vcf-string (if use-quoted-printable
                                      (alist->vcf3.0-format-qprint alist output)
                                      (alist->vcf3.0-format alist)))
                      (qr-png-filepath (make-pathname :type "png"
                                                      :name (get-vcf-field-value 'full-name alist)
                                                      :defaults csv-filepath)))
                  (format t "~A~%" vcf-string)
                  (format output "~A~%~%" vcf-string)
                  (if gen-qr
                      (cl-qrencode:encode-png-bytes
                       (map 'list
                            #'identity
                            (flexi-streams:string-to-octets
                             vcf-string
                             :external-format :utf-8))
                       :fpath qr-png-filepath
                       :pixsize 3
                       :level :level-l))))))))
